...
final class UserController extends AbstractController
{
    ...
    public function userRegistration(Request $request, UserPasswordHasherInterface $passwordHasher): Response
    {
        ...
        if ($registration_form->isSubmitted() && $registration_form->isValid()) { 
            ...
            if ($photoFile) {
                $destination = $this->getParameter('kernel.project_dir') . '/public/uploads/profiles'; // Ruta donde se guardará la imagen
                $newFilename = uniqid() . '.' . $photoFile->guessExtension(); // Nombre único para la imagen
